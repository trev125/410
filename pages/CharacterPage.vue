  ...
  >
    ...
    >
      ...
      <v-card v-if="!hasCharacter">
        ...
        <v-col cols="12">
          <v-subheader>Dexterity</v-subheader>
          <v-slider
            v-model="dexSlider"
            thumb-label="always"
            min="0"
        ...
        <v-col cols="12">
          <v-subheader>Speech</v-subheader>
          <v-slider
            v-model="speechSlider"
            thumb-label="always"
            min="0"
            max="10"
            height="5"
        ...
        <v-col cols="12">
          <v-subheader>Intelligence</v-subheader>
          <v-slider
            v-model="intelSlider"
            thumb-label="always"
            min="0"
        ...
        <v-dialog v-model="submitWithExtraPoints" max-width="400">
          <v-card>
            <v-card-title class="headline">Confirm Creation</v-card-title>
            <v-card-text>
              You still have extra skill points to use! Are you sure you want to create this character?
              <br>Points used: {{this.totalPointsUsed}}/25.
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="green darken-1" text @click="submitWithExtraPoints = false"
              >
                No
              </v-btn>
              <v-btn color="green darken-1" text
                @click="createCharacter(), submitWithExtraPoints = false"
              >
                Yes
              </v-btn>
            </v-card-actions>
        ...
        <v-dialog v-model="submitWithAllPoints" max-width="400">
          <v-card>
            <v-card-title class="headline">Confirm Creation</v-card-title>
            <v-card-text>
              ...
              <v-row>
                <v-col cols="6">
                  Strength: {{strengthSlider}}/10
                </v-col>
                <v-col cols="6">
                  Dexterity: {{dexSlider}}/10
                </v-col>
              </v-row>
              <v-row>
                <v-col cols="6">
                  Speech: {{speechSlider}}/10
                </v-col>
                <v-col cols="6">
                  Intelligence: {{intelSlider}}/10
                </v-col>
              </v-row>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="green darken-1" text @click="submitWithAllPoints = false"
              >
                No
              </v-btn>
              <v-btn color="green darken-1" text
                @click="createCharacter(), submitWithAllPoints = false"
              >
                Yes
              </v-btn>
            </v-card-actions>
        ...
        <v-card-actions>
          <v-btn color="primary" :disabled="hasUsedMorePoints" v-if="hasUsedAllPoints" @click.stop="submitWithAllPoints = true">
            Create Character
          </v-btn>
          <v-btn color="primary" :disabled="hasUsedMorePoints" @click.stop="submitWithExtraPoints = true" v-else>
            Create with unused points
          </v-btn>
        </v-card-actions>
      </v-card>
      <v-card v-else>
...
export default {
  data() {
    return {
      characterName: '',
      nameRules: [v => v.length <= 25 || 'Max 25 characters'],
      hasCharacter: false,
      strengthSlider: 5,
      dexSlider: 5,
      speechSlider: 5,
  ...
  methods: {
    checkTotal: function(){
      ...
      this.totalPointsUsed = totalPoints;
      if (totalPoints > 25 || this.characterName.length > 25){
        console.log("ERROR!!", totalPoints);
        this.hasUsedAllPoints = true;
        this.hasUsedMorePoints = true;
      }
      else if(totalPoints === 25){
        console.log("All points used!!", totalPoints);
        this.hasUsedMorePoints = false;
        this.hasUsedAllPoints = true;
      }
      else {
        if (this.characterName.length > 25){
          this.hasUsedAllPoints = true;
          this.hasUsedMorePoints = true;
        }
        else{
          this.hasUsedAllPoints = false;
          this.hasUsedMorePoints = false;
        }
      }
    },
